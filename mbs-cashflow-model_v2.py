...
def amortization(balance, rate, terms, cdr, psa_speed, lag, loss):
    n_loans = len(balance)
    n_month = terms.max()    
    ...
    return pool_begbal, pool_int, pool_principal, pool_default, pool_prepay, pool_loss, pool_recovery, pool_endbal
...
def waterfall(pool_begbal, pool_int, pool_principal, pool_default, pool_prepay, pool_loss, pool_recovery, pool_endbal):
...
def psa_scenario(balance, rate, terms, cdr, psa_speeds, lag, loss_sev):
    wal_output = np.zeros((len(psa_speeds), 3))
    for i in range(len(psa_speeds)):
        psa = psa_speeds[i]
        pool_begbal, pool_int, pool_principal, pool_default, pool_prepay, pool_loss, pool_recovery, pool_endbal = amortization(balance, rate, terms, annual_cdr, psa, lag, loss_sev)
        tranche = waterfall(pool_begbal, pool_int, pool_principal, pool_default, pool_prepay, pool_loss, pool_recovery, pool_endbal)
        wal_i = wal(tranche)
        wal_output[i,:] = wal_i
    return wal_output 

'''Start Of inputs and structure'''
...
#Calling of amortization function
pool_begbal, pool_int, pool_principal, pool_default, pool_prepay, pool_loss, pool_recovery, pool_endbal = amortization(balance, rate, terms, annual_cdr, psa_speed, lag, loss_sev)

#Creating the correct structure to export CSV
...
water = waterfall(*amortization(balance, rate, terms, annual_cdr, psa_speed, lag, loss_sev))

psa_speeds = np.array((50, 75, 100, 150, 200, 250))

sa_psa = psa_scenario(balance, rate, terms, annual_cdr, psa_speeds, lag, loss_sev)
